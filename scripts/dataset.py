...
def create_dataset():
    img_db_fn = 'data/image_occ_train.lmdb'
    ...
    img_fns = glob.glob('data/cad60_dataset/crop/*_OCC.jpg')
    jnt_fns = glob.glob('data/cad60_dataset/joint/*.npy')
    #Curate the img_fns so that sorting becomes easier
    img_fns = [filename[:-8] for filename in img_fns]
    for i, (img_fn, jnt_fn) in enumerate(
            zip(sorted(img_fns), sorted(jnt_fns))):
        img_fn = img_fn+'_OCC.jpg'
